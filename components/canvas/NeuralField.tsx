...
function Field() {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      pos[i * 3] += velArr[i * 3]
      pos[i * 3 + 1] += velArr[i * 3 + 1] + Math.sin(t * 0.3 + i) * 0.0005
    ...
    // Build connection lines without allocation
  ...
  return (
    <>
      <points ref={pointsRef} geometry={pointGeo}>
        <pointsMaterial color="#00ff41" size={0.035} transparent opacity={0.75} sizeAttenuation />
      </points>
